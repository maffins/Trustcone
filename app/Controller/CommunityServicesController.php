 ...
 */
class CommunityServicesController extends AppController {
        ...
        public function committeedetails() {
      	...
      	public function messageresend() {
      			if ($this->request->is('post')) {
      				$message = $this->request->data['CommunityService']['message'];
      				$subject = $this->request->data['CommunityService']['subject'];

      				if(!$subject)
      				{
      					$subject = "Community Service Section 80 Meeting documents are posted documents are posted.";
      				}
      				if(!$message)
      				{
      					$message = "Community Service Section 80 Meeting documents are posted documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
      				}

      				if($this->request->data['CommunityService']['sendsms']){
      					 $this->sendsms($message);
      				}
      				if($this->request->data['CommunityService']['sendemail']){
      				...
      				return $this->redirect(array('Controller' => 'CommunityService', 'action' => 'committeedetails'));
      		}
      	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, created, idcounter, user_id', 'conditions'=>['Meeting.type' => 4]]);
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened the meetings page for Community Service Section 80 ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing all Community Service Section 80 documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 1)
        {
            ...
            $directory = "community_service_agenda";
        }
        ...
        if($which == 2)
        {
            ...
            $directory = "community_service_minutes";
        }
        ...
        if($which == 3)
        {
            ...
            $tafura    = "MeetingItem";
            $directory = "community_service_items";
        }
        ...
        if($which == 4)
        {
            ...
            $directory = "community_service_attachments";
        }
        ...
        if($which == 5)
        {
            ...
            $directory = "community_service_separateCovers";
        }
        ...
        if($which == 8)
        {
            ...
            $directory = "community_service_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 4));
...
// a view
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Opened to view document Community Service Section 80with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Opened to view Community Service Section 80 document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Community Service Section 80 document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 4));
// $options = array('conditions' => array('MeetingAgenda.type' => 2));
        ...
        */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 4));
        ...
        */
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 4));
        ...
        */
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 4));
        ...
        */
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 4));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 4));
        ...
        $this->set('MeetingSeparatecovers', $MeetingSeparateCovers);
				 ...
				 */
				$this->loadModel('MeetingNotice');
				$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 4));
				 ...
				 */
				$this->loadModel('MeetingAddendum');
				$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 4));
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Viewing the meeting details for Community Service Section 80 meeting id ".$id." ip ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Viewing the meeting details for Community Service Section 80 meeting id ".$id." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Community Service Section 80 document');
        }
...
//First save the meeting name
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['CommunityService']['meeting'];
            $meeting['Meeting']['type']    = 4;            //Before saving the meeting, get the previous id counter and increment it then save.
            //Retrieve it and most important is the type when retrieving then afterwards simply add 1
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 4),
...
//First save the meeting agenda
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['CommunityService']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_agenda' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $agendaDetails['MeetingAgenda']['type']       = 4;
...
//First save the meeting previous minutes
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['CommunityService']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_minutes' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 4;
...
//First save the meeting items
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['CommunityService']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_items' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 4;
...
//First save the meeting attachments
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['CommunityService']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_attachments' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 4;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['CommunityService']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach ($separateCoversDocuments as $upload) {
                ...
                if ($meeting_id) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type'] = 4;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['CommunityService']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'community_service_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 4;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a Community Service Section 80 document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a Community Service Section 80 document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding Community Service Section 80 document');
            }

            //This is for sending emails to others not part of counciler user type
  					if($notifications) {
  						if($sendsms) {
  							$this->sendsms($this->request->data['ChairDocument']['message']);
  						}
  						if($sendemail) {
  	            $this->sendemail($this->request->data['ChairDocument']['message'], $this->request->data['ChairDocument']['subject']);
  						}
            } else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'CommunityService', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add Community Service Section 80 add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add Community Service Section 80 document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing Community Service Section 80 document');
        }

        $users = $this->CommunityService->User->find('list');
    ...
    function sendsms($message=null)
    {
        ...
        $smsText = urlencode("Community Service Section 80 Meeting documents are posted documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.");
        ...
        if(!$message)
        {
          $message = "Community Service Section 80 Commitee Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    function sendemail($message=null, $subject=null)
    {
        ...
        if(!$subject)
        {
          $subject = "Community Service Section 80 Commitee Meeting documents are posted";
        }
        if(!$message)
        {
          $message = "Community Service Section 80 Commitee Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    ...
    public function cantdownload($id, $which) {
        ...
        if($which == 7)
        {
            $this->loadModel('MeetingNotice');
            $tafura    = "MeetingNotice";
            $directory = "community_service_notice";
            $docname   = "Minutes";
        }
...
//Get the meeting name
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 4));
...
//Send the notification to the logged in user.
        $Email->from(array('no-reply@documentstracker.com' => 'Matjhabeng Local Municipality Document Management System'))
            ->template('cantdownload_communityservice ', 'default')
            ->emailFormat('html')
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Community Service Section 80 member tried to view document ".$docname." with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR']." But was not successful";
    ...
    public function edit($id = null) {
        if (!$this->CommunityService->exists($id)) {
            throw new NotFoundException(__('Invalid Community Service Section 80 document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->CommunityService->save($this->request->data)) {
                $this->Session->setFlash(__('The Community Service Section 80 document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Community Service Section 80 document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('CommunityService.' . $this->CommunityService->primaryKey => $id));
            $this->request->data = $this->CommunityService->find('first', $options);
        }
        $users = $this->CommunityService->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->CommunityService->id = $id;
        if (!$this->CommunityService->exists()) {
            throw new NotFoundException(__('Invalid Community Service Section 80 document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->CommunityService->delete()) {
            $this->Session->setFlash(__('The Community Service Section 80 document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The Community Service Section 80 document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['CommunityService']['meeting_id'];
...
//First save the meeting previous minutes
            ...
            foreach ($minutesDocuments as $upload) {
                ...
                if ($meeting_id) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'Community Service_minutes' . DS . $file['name']);
//prepare the filename for database entry
                    $minutesDetails['MeetingMinute']['document_name'] = $file['name'];
                    $minutesDetails['MeetingMinute']['original_name'] = $original_name;
                } else {
...
//First save the meeting items
            ...
            foreach ($itemsDocuments as $upload) {
                ...
                if ($meeting_id) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'Community Service_items' . DS . $file['name']);
//prepare the filename for database entry
                    $itemsDetails['MeetingItem']['document_name'] = $file['name'];
                    $itemsDetails['MeetingItem']['original_name'] = $original_name;
                } else {
...
//First save the meeting attachments
            ...
            foreach ($attachmentDocuments as $upload) {
                ...
                if ($meeting_id) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'Community Service_attachments' . DS . $file['name']);
//prepare the filename for database entry
                    $attachmentDetails['MeetingAttachment']['document_name'] = $file['name'];
                    $attachmentDetails['MeetingAttachment']['original_name'] = $original_name;
                } else {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 4));
