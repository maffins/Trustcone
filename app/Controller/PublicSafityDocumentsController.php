...
/**
 * PublicSafityDocuments Controller
 *
 * @property PublicSafityDocument $PublicSafityDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class PublicSafityDocumentsController extends AppController {
    ...
    public function viewmembers() {
  		...
  		{
  				if( in_array(7, unserialize($user['User']['permissions'])) )
  				{
  						$allmembers[] = $user;
  				}
  		}
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['PublicSafityDocument']['message'];
    				$subject = $this->request->data['PublicSafityDocument']['subject'];

    				if(!$subject)
    				{
    					$subject = "PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['PublicSafityDocument']['sendsms']){
    					 $this->sendsms($message);
    				}
    				if($this->request->data['PublicSafityDocument']['sendemail']){
    				...
    				return $this->redirect(array('Controller' => 'PublicSafityDocument', 'action' => 'committeedetails'));
    		}
    	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 9]]);
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened the meetings page for PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing all PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 1)
        {
            ...
            $directory = "public_safity_agenda";
        }
        ...
        if($which == 2)
        {
            ...
            $directory = "public_safity_minutes";
        }
        ...
        if($which == 4)
        {
            ...
            $directory = "public_safity_attachments";
        }
        ...
        if($which == 5)
        {
            ...
            $directory = "public_safity_separatecovers";
        }
        ...
        if($which == 7)
        {
            ...
            $directory = "public_safity_notice";
        }
        ...
        if($which == 8)
        {
            ...
            $directory = "public_safity_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 9));
...
// a view
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Opened to view document PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEEwith id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Opened to view PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEESection 80 meeting document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 9));
// $options = array('conditions' => array('MeetingAgenda.type' => 2));
        ...
        */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 9));
        ...
        */
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 9));
        ...
        */
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 9));
        ...
        */
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 9));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 9));
         ...
         */
        $this->loadModel('MeetingNotice');
        $options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 9));
         ...
         */
        $this->loadModel('MeetingAddendum');
        $options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 9));
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Viewing the meeting details for PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE meeting id ".$id." ip ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Viewing the meeting details for PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE meeting id ".$id." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document');
        }
...
//First save the meeting name
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['PublicSafityDocument']['meeting'];
            $meeting['Meeting']['type']    = 9;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 9),
...
//First save the meeting agenda
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['PublicSafityDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_agenda' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $agendaDetails['MeetingAgenda']['type']       = 9;
...
//First save the meeting previous minutes
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['PublicSafityDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_minutes' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 9;
...
//First save the meeting items
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['PublicSafityDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_items' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 9;
...
//First save the meeting attachments
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['PublicSafityDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_attachments' . DS . $file['name']);
//prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 9;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['PublicSafityDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 9;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['PublicSafityDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'public_safity_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 9;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document');
            }

            $notifications = $this->request->data['PublicSafityDocument']['notifications'];
            unset($this->request->data['PublicSafityDocument']['notifications']);

            $sendemail = $this->request->data['PublicSafityDocument']['sendemail'];
            unset($this->request->data['PublicSafityDocument']['sendemail']);

            $sendsms = $this->request->data['PublicSafityDocument']['sendsms'];
            unset($this->request->data['PublicSafityDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['PublicSafityDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['PublicSafityDocument']['message'], $this->request->data['PublicSafityDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'PublicSafityDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document');
        }

       // $users = $this->PublicSafityDocument->User->find('list');
...
//Get the meeting name
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 9));
...
//Send the notification to the logged in user.
        $Email->from(array('no-reply@documentstracker.com' => 'Matjhabeng Local Municipality Document Management System'))
            ->template('cantdownload_PublicSafityDocument ', 'default')
            ->emailFormat('html')
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE member tried to view document ".$docname." with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR']." But was not successful";
    ...
    public function edit($id = null) {
        if (!$this->PublicSafityDocument->exists($id)) {
            throw new NotFoundException(__('Invalid PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->PublicSafityDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('PublicSafityDocument.' . $this->PublicSafityDocument->primaryKey => $id));
            $this->request->data = $this->PublicSafityDocument->find('first', $options);
        }
        $users = $this->PublicSafityDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->PublicSafityDocument->id = $id;
        if (!$this->PublicSafityDocument->exists()) {
            throw new NotFoundException(__('Invalid PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->PublicSafityDocument->delete()) {
            $this->Session->setFlash(__('The PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The PUBLIC SAFETY AND TRANSPORT SECTION 80 COMMITTEE document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['PublicSafityDocument']['meeting_id'];
...
//prepare the filename for database entry
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 9));
