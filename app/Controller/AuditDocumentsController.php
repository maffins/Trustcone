...
/**
 * AuditDocuments Controller
 *
 * @property AuditDocument $AuditDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class AuditDocumentsController extends AppController {
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['AuditDocument']['message'];
    				$subject = $this->request->data['AuditDocument']['subject'];

    				if(!$subject)
    				{
    					$subject = "AUDIT COMMITTEE Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "AUDIT COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['AuditDocument']['sendsms']){
    					 $this->sendsms($message);
    				}
    				if($this->request->data['AuditDocument']['sendemail']){
    				...
    				return $this->redirect(array('Controller' => 'AuditDocument', 'action' => 'committeedetails'));
    		}
    	}
...
public function viewmembers() {
  ...
  {
      if( in_array(74, unserialize($user['User']['permissions'])) )
      {
          $allmembers[] = $user;
      }
  }
  ...
  $this->set('allusers', $allmembers);
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 18]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all Audit Committee documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 5)
        {
            ...
            $directory = "audit_separateCovers";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 18));
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view Audit Committee document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Audit Committee document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 18));
        // $options = array('conditions' => array('MeetingAgenda.type' => 18));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 18));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 18));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 18));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 18));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 18));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['AuditDocument']['meeting'];
            $meeting['Meeting']['type']    = 18;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 18),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['AuditDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'audit_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 18;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['AuditDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'audit_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 18;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['AuditDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'audit_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 18;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['AuditDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'audit_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 18;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['AuditDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'audit_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 18;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a Audit Committee document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a Audit Committee document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding Audit Committee document');
            }

            $notifications = $this->request->data['AuditDocument']['notifications'];
  					unset($this->request->data['AuditDocument']['notifications']);

  					$sendemail = $this->request->data['AuditDocument']['sendemail'];
  					unset($this->request->data['AuditDocument']['sendemail']);

  					$sendsms = $this->request->data['AuditDocument']['sendsms'];
  					unset($this->request->data['AuditDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
  					if($notifications) {
  						if($sendsms) {
  							$this->sendsms($this->request->data['AuditDocument']['message']);
  						}
  						if($sendemail) {
  	            $this->sendemail($this->request->data['AuditDocument']['message'], $this->request->data['AuditDocument']['subject']);
  						}
            } else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'AuditDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add Audit Committee add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add Audit Committee document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing Audit Committee document');
        }

        $users = $this->AuditDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 18));
    ...
    public function edit($id = null) {
        if (!$this->AuditDocument->exists($id)) {
            throw new NotFoundException(__('Invalid Audit Committee document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->AuditDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The Audit Committee document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Audit Committee document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('AuditDocument.' . $this->AuditDocument->primaryKey => $id));
            $this->request->data = $this->AuditDocument->find('first', $options);
        }
        $users = $this->AuditDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->AuditDocument->id = $id;
        if (!$this->AuditDocument->exists()) {
            throw new NotFoundException(__('Invalid Audit Committee document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->AuditDocument->delete()) {
            $this->Session->setFlash(__('The Audit Committee document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The Audit Committee document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['AuditDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 18));
