...
/**
 * HumanDocuments Controller
 *
 * @property HumanDocument $HumanDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class HumanDocumentsController extends AppController {
  /**
   * Components
   *
   * @var array
  ...
  public function viewmembers() {
    ...
    {
        if( in_array(56, unserialize($user['User']['permissions'])) )
        {
            $allmembers[] = $user;
        }
    }
    ...
    public function committeedetails() {
  	...
  	public function messageresend() {
  			if ($this->request->is('post')) {
  				$message = $this->request->data['HumanDocument']['message'];
  				$subject = $this->request->data['HumanDocument']['subject'];

  				if(!$subject)
  				{
  					$subject = "Human Settlement Committee Meeting documents are posted.";
  				}
  				if(!$message)
  				{
  					$message = "Human Settlement Committee Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
  				}

  				if($this->request->data['HumanDocument']['sendsms']){
  					 $this->sendsms($message);
  				}
  				if($this->request->data['HumanDocument']['sendemail']){
  				...
  				return $this->redirect(array('Controller' => 'HumanDocument', 'action' => 'committeedetails'));
  		}
  	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 15]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all Human settlement documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 5)
        {
            ...
            $directory = "human_separatecovers";
        }
        ...
        if($which == 8)
        {
            ...
            $directory = "human_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 15));
        //print_r($options);
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view Human settlement document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Human settlement document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 15));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 15));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 15));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 15));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 15));
   ...
   */
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 15));
        ...
        $this->set('MeetingSeparatecovers', $MeetingSeparateCovers);
  			 ...
  			 */
  			$this->loadModel('MeetingNotice');
  			$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 15));
  			 ...
  			 */
  			$this->loadModel('MeetingAddendum');
  			$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 15));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['HumanDocument']['meeting'];
            $meeting['Meeting']['type']    = 15;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 15),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['HumanDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 15;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['HumanDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 15;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['HumanDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 15;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['HumanDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 15;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['HumanDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_separatecovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 15;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['HumanDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'human_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 15;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a Human settlement document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a Human settlement document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding Human settlement document');
            }

            $notifications = $this->request->data['HumanDocument']['notifications'];
            unset($this->request->data['HumanDocument']['notifications']);

            $sendemail = $this->request->data['HumanDocument']['sendemail'];
            unset($this->request->data['HumanDocument']['sendemail']);

            $sendsms = $this->request->data['HumanDocument']['sendsms'];
            unset($this->request->data['HumanDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['HumanDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['HumanDocument']['message'], $this->request->data['HumanDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'HumanDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add Human settlement add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add Human settlement document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing Human settlement document');
        }

        $users = $this->HumanDocument->User->find('list');
    ...
    function sendsms($message = null)
    {
        ...
        if(!$message)
        {
          $message = "Human Settlement Committee documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    function sendemail($message = null, $subject = null)
    {
        ...
        $users = $this->User->find('all');
				...
				if(!$subject)
				{
          $subject = "Human Settlement Committee documents are posted";
				}
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 15));
        ...
        $Email->from(array('no-reply@documentstracker.com' => 'Matjhabeng Local Municipality Document Management System'))
            ->template('cantdownload_Human settlement', 'default')
            ->emailFormat('html')
    ...
    public function edit($id = null) {
        if (!$this->HumanDocument->exists($id)) {
            throw new NotFoundException(__('Invalid Human settlement document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->HumanDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The Human settlement document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Human settlement document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('HumanDocument.' . $this->HumanDocument->primaryKey => $id));
            $this->request->data = $this->HumanDocument->find('first', $options);
        }
        $users = $this->HumanDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->HumanDocument->id = $id;
        if (!$this->HumanDocument->exists()) {
            throw new NotFoundException(__('Invalid Human settlement document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->HumanDocument->delete()) {
            $this->Session->setFlash(__('The Human settlement document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The Human settlement document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['HumanDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 15));
