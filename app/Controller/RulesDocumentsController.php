...
/**
 * RulesDocuments Controller
 *
 * @property RulesDocument $RulesDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class RulesDocumentsController extends AppController {
    /**
     * Components
     *
     * @var array
    ...
    public function viewmembers() {
  		...
  		{
  				if( in_array(16, unserialize($user['User']['permissions'])) )
  				{
  						$allmembers[] = $user;
  				}
  		}
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['RulesDocument']['message'];
    				$subject = $this->request->data['RulesDocument']['subject'];

    				if(!$subject)
    				{
    					$subject = "RULES SECTION 79 COMMITTEE Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "RULES SECTION 79 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['RulesDocument']['sendsms']){
    					 $this->sendsms($message);
    				}
    				if($this->request->data['RulesDocument']['sendemail']){
    				...
    				return $this->redirect(array('Controller' => 'RulesDocument', 'action' => 'committeedetails'));
    		}
    	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 12]]);
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 5)
        {
    		...
    		if($which == 8)
    		{
    				...
    				$directory = "rules_addendum";
    		}

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 12));
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 12));
        // $options = array('conditions' => array('MeetingAgenda.type' => 12));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 12));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 12));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 12));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 12));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 12));
        ...
        /*
    		 ...
    		 */
    		$this->loadModel('MeetingNotice');
    		$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 12));
    		 ...
    		 */
    		$this->loadModel('MeetingAddendum');
    		$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 12));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['RulesDocument']['meeting'];
            $meeting['Meeting']['type']    = 12;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 12),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['RulesDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 12;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['RulesDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 12;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['RulesDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 12;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['RulesDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 12;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['RulesDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 12;
                ...
                if ($this->MeetingSeparatecover->save($separeCoverDetails)) {
    				...
    				$this->loadModel('MeetingAddendum');
    				$addendumDocuments = $this->request->data['RulesDocument']['addendum'];
    				$addendumDetails = [];
    				...
    				foreach( $addendumDocuments as $upload)
    				{
    						...
    						if ($meetingName) {
    								...
    								$original_name = $file['name'];
    								$file['name'] = preg_replace('/\s+/', '_', $file['name']);
    								move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'rules_addendum' . DS . $file['name']);
    								//prepare the filename for database entry
    						...
    						$addendumDetails['MeetingAddendum']['type']       = 12;
            ...
            $notifications = $this->request->data['RulesDocument']['notifications'];
            unset($this->request->data['RulesDocument']['notifications']);

            $sendemail = $this->request->data['RulesDocument']['sendemail'];
            unset($this->request->data['RulesDocument']['sendemail']);

            $sendsms = $this->request->data['RulesDocument']['sendsms'];
            unset($this->request->data['RulesDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['RulesDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['RulesDocument']['message'], $this->request->data['RulesDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'RulesDocument', 'action' => 'index'));
        }
        ...
        $users = $this->RulesDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 12));
    ...
    public function edit($id = null) {
        if (!$this->RulesDocument->exists($id)) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->RulesDocument->save($this->request->data)) {
            ...
            $options = array('conditions' => array('RulesDocument.' . $this->RulesDocument->primaryKey => $id));
            $this->request->data = $this->RulesDocument->find('first', $options);
        }
        $users = $this->RulesDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->RulesDocument->id = $id;
        if (!$this->RulesDocument->exists()) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->RulesDocument->delete()) {
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['RulesDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 12));
