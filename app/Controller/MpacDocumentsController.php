...
/**
 * MpacDocuments Controller
 *
 * @property MpacDocument $MpacDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class MpacDocumentsController extends AppController {
    ...
    public function viewmembers() {
  		...
  		{
  				if( in_array(14, unserialize($user['User']['permissions'])) )
  				{
  						$allmembers[] = $user;
  				}
  		}
    ...
    public function messageresend() {
        if ($this->request->is('post')) {
          $message = $this->request->data['MpacDocument']['message'];
          $subject = $this->request->data['MpacDocument']['subject'];

          if(!$subject)
          {
            $subject = "MPAC SECTION 79 COMMITTEE Meeting documents are posted.";
          }
          if(!$message)
          {
            $message = "MPAC SECTION 79 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
          }

          if($this->request->data['MpacDocument']['sendsms']){
             $this->sendsms($message);
          }
          if($this->request->data['MpacDocument']['sendemail']){
          ...
          return $this->redirect(array('Controller' => 'MpacDocuments', 'action' => 'committeedetails'));
      }
    }
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 11]]);
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 8)
        {
            ...
            $directory = "mpac_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 11));
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 11));
        // $options = array('conditions' => array('MeetingAgenda.type' => 11));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 11));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 11));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 11));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 11));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 11));
        ...
        /*
    		 ...
    		 */
    		$this->loadModel('MeetingNotice');
    		$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 11));
    		 ...
    		 */
    		$this->loadModel('MeetingAddendum');
    		$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 11));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['MpacDocument']['meeting'];
            $meeting['Meeting']['type']    = 11;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 11),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['MpacDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 11;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['MpacDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 11;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['MpacDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 11;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['MpacDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 11;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['MpacDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 11;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['MpacDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'mpac_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 11;
            ...
            $notifications = $this->request->data['MpacDocument']['notifications'];
            unset($this->request->data['MpacDocument']['notifications']);

            $sendemail = $this->request->data['MpacDocument']['sendemail'];
            unset($this->request->data['MpacDocument']['sendemail']);

            $sendsms = $this->request->data['MpacDocument']['sendsms'];
            unset($this->request->data['MpacDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['MpacDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['MpacDocument']['message'], $this->request->data['MpacDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'MpacDocument', 'action' => 'index'));
        }
        ...
        $users = $this->MpacDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 11));
    ...
    public function edit($id = null) {
        if (!$this->MpacDocument->exists($id)) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->MpacDocument->save($this->request->data)) {
            ...
            $options = array('conditions' => array('MpacDocument.' . $this->MpacDocument->primaryKey => $id));
            $this->request->data = $this->MpacDocument->find('first', $options);
        }
        $users = $this->MpacDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->MpacDocument->id = $id;
        if (!$this->MpacDocument->exists()) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->MpacDocument->delete()) {
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['MpacDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 11));
