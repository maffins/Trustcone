...
/**
 * LedDocuments Controller
 *
 * @property LedDocument $LedDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class LedDocumentsController extends AppController {
    ...
    public function viewmembers() {
      ...
      {
          if( in_array(53, unserialize($user['User']['permissions'])) )
          {
              $allmembers[] = $user;
          }
      }
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['LedDocument']['message'];
    				$subject = $this->request->data['LedDocument']['subject'];

    				if(!$subject)
    				{
    					$subject = "Led, Small Business, Spatial Planning and Land Use Management Committee Section 80 Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "Led, Small Business, Spatial Planning and Land Use Management Committee Section 80 Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['LedDocument']['sendemail']){
    					 $this->sendemail($message, $subject);
    				}
            if($this->request->data['LedDocument']['sendsms']){
    					 $this->sendsms($message);
    				}
    				$this->Session->setFlash(__('Your messages have been sent.'), 'default', array('class' => 'alert alert-success'));

    				return $this->redirect(array('Controller' => 'LedDocument', 'action' => 'committeedetails'));
    		}
    	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['order' => ['Meeting.id' => 'DESC'], 'fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 16]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all Led, Small Business, Spatial Planning and Land Use Management documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 8)
        {
            ...
            $directory = "led_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 16));
        //print_r($options);
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view Led, Small Business, Spatial Planning and Land Use Management document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Led, Small Business, Spatial Planning and Land Use Management document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 16));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 16));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 16));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 16));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 16));
   ...
   */
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 16));
        ...
        /*
    		 ...
    		 */
    		$this->loadModel('MeetingNotice');
    		$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 16));
    		 ...
    		 */
    		$this->loadModel('MeetingAddendum');
    		$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 16));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['LedDocument']['meeting'];
            $meeting['Meeting']['type']    = 16;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 16),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['LedDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 16;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['LedDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 16;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['LedDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 16;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['LedDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 16;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['LedDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_separatecovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 16;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['LedDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'led_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 16;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a Led, Small Business, Spatial Planning and Land Use Management document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a Led, Small Business, Spatial Planning and Land Use Management document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding Led, Small Business, Spatial Planning and Land Use Management document');
            }

            $notifications = $this->request->data['LedDocument']['notifications'];
            unset($this->request->data['LedDocument']['notifications']);

            $sendemail = $this->request->data['LedDocument']['sendemail'];
            unset($this->request->data['LedDocument']['sendemail']);

            $sendsms = $this->request->data['LedDocument']['sendsms'];
            unset($this->request->data['LedDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['LedDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['LedDocument']['message'], $this->request->data['LedDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'LedDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add Led, Small Business, Spatial Planning and Land Use Management add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add Led, Small Business, Spatial Planning and Land Use Management document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing Led, Small Business, Spatial Planning and Land Use Management document');
        }

        $users = $this->LedDocument->User->find('list');
    ...
    function sendsms($message = null)
    {
        ...
        $numbers .= '27823087961,27817549884';

        if(!$message)
        {
          $message = "Led, Small Business, Spatial Planning and Land Use Management documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    function sendemail($message = null, $subject = null)
    {
        ...
        if(!$subject)
        {
          $subject = "Led, Small Business, Spatial Planning and Land Use Management Committee documents are posted";
        }
        if(!$message)
        {
          $message = "Led, Small Business, Spatial Planning and Land Use Managemente Committee documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    public function cantdownload($id, $which) {
        ...
        if($which == 5)
        {
            $this->loadModel('MeetingSeparatecover');
            $tafura    = "MeetingSeparatecover";
            $directory = "MeetingSeparatecover";
            $docname   = "MeetingSeparatecover";
        }
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 16));
        ...
        $Email->from(array('no-reply@documentstracker.com' => 'Matjhabeng Local Municipality Document Management System'))
            ->template('cantdownload_Led, Small Business, Spatial Planning and Land Use Management', 'default')
            ->emailFormat('html')
    ...
    public function edit($id = null) {
        if (!$this->LedDocument->exists($id)) {
            throw new NotFoundException(__('Invalid Led, Small Business, Spatial Planning and Land Use Management document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->LedDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The Led, Small Business, Spatial Planning and Land Use Management document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Led, Small Business, Spatial Planning and Land Use Management document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('LedDocument.' . $this->LedDocument->primaryKey => $id));
            $this->request->data = $this->LedDocument->find('first', $options);
        }
        $users = $this->LedDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->LedDocument->id = $id;
        if (!$this->LedDocument->exists()) {
            throw new NotFoundException(__('Invalid Led, Small Business, Spatial Planning and Land Use Management document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->LedDocument->delete()) {
            $this->Session->setFlash(__('The Led, Small Business, Spatial Planning and Land Use Management document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The Led, Small Business, Spatial Planning and Land Use Management document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['LedDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 16));
