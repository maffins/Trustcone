...
/**
 * DisputeResolutionDocuments Controller
 *
 * @property DisputeResolutionDocument $DisputeResolutionDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class DisputeResolutionDocumentsController extends AppController {
  ...
  public function viewmembers() {
		...
		{
				if( in_array(15, unserialize($user['User']['permissions'])) )
				{
						$allmembers[] = $user;
				}
		}
    ...
    public function committeedetails() {
  	...
  	public function messageresend() {
  			if ($this->request->is('post')) {
  				$message = $this->request->data['DisputeResolutionDocument']['message'];
  				$subject = $this->request->data['DisputeResolutionDocument']['subject'];

  				if(!$subject)
  				{
  					$subject = "DISPUTE RESOLUTION SECTION 79 COMMITTEE Meeting documents are posted.";
  				}
  				if(!$message)
  				{
  					$message = "DISPUTE RESOLUTION SECTION 79 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
  				}

  				if($this->request->data['DisputeResolutionDocument']['sendsms']){
  					 $this->sendsms($message);
  				}
  				if($this->request->data['DisputeResolutionDocument']['sendemail']){
  				...
  				return $this->redirect(array('Controller' => 'DisputeResolutionDocument', 'action' => 'committeedetails'));
  		}
  	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, created, user_id', 'conditions'=>['Meeting.type' => 13]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all mpac documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 1)
        {
            ...
            $directory = "dispute_resolution_agenda";
        }
        ...
        if($which == 2)
        {
            ...
            $directory = "dispute_resolution_minutes";
        }
        ...
        if($which == 3)
        {
            ...
            $tafura    = "MeetingItem";
            $directory = "dispute_resolution_items";
        }
        ...
        if($which == 4)
        {
            ...
            $directory = "dispute_resolution_attachments";
        }
        ...
        if($which == 5)
        {
            ...
            $directory = "dispute_resolution_separatecovers";
        }
        ...
        if($which == 8)
        {
            ...
            $directory = "dispute_resolution_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 13));
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view mpac document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 13));
        // $options = array('conditions' => array('MeetingAgenda.type' => 13));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 13));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 13));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 13));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 13));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 13));
        ...
        $MeetingSeparateCovers = $this->MeetingSeparatecovers->find('all', $options5);
				 ...
				 */
				$this->loadModel('MeetingNotice');
				$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 13));
				 ...
				 */
				$this->loadModel('MeetingAddendum');
				$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 13));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['DisputeResolutionDocument']['meeting'];
            $meeting['Meeting']['type']    = 13;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 13),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['DisputeResolutionDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 13;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['DisputeResolutionDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 13;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['DisputeResolutionDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 13;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['DisputeResolutionDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 13;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['DisputeResolutionDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 13;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['DisputeResolutionDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'dispute_resolution_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 13;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a mpac document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a mpac document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding mpac document');
            }

            $notifications = $this->request->data['DisputeResolutionDocument']['notifications'];
  					unset($this->request->data['DisputeResolutionDocument']['notifications']);

  					$sendemail = $this->request->data['DisputeResolutionDocument']['sendemail'];
  					unset($this->request->data['DisputeResolutionDocument']['sendemail']);

  					$sendsms = $this->request->data['DisputeResolutionDocument']['sendsms'];
  					unset($this->request->data['DisputeResolutionDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
  					if($notifications) {
  						if($sendsms) {
  							$this->sendsms($this->request->data['DisputeResolutionDocument']['message']);
  						}
  						if($sendemail) {
  	            $this->sendemail($this->request->data['DisputeResolutionDocument']['message'], $this->request->data['DisputeResolutionDocument']['subject']);
  						}
  					}else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'DisputeResolutionDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add mpac add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add mpac document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing mpac document');
        }

        $users = $this->DisputeResolutionDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        if($which == 2)
        {
            $this->loadModel('MeetingMinute');
            $tafura    = "MeetingMinute";
            $directory = "dispute_resolutionminutes";
            $docname   = "Minutes";
        }
        ...
        if($which == 7)
        {
            $this->loadModel('MeetingNotice');
            $tafura    = "MeetingNotice";
            $directory = "dispute_resolution_notice";
            $docname   = "Minutes";
        }
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 13));
    ...
    public function edit($id = null) {
        if (!$this->DisputeResolutionDocument->exists($id)) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->DisputeResolutionDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The mpac document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The mpac document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('DisputeResolutionDocument.' . $this->DisputeResolutionDocument->primaryKey => $id));
            $this->request->data = $this->DisputeResolutionDocument->find('first', $options);
        }
        $users = $this->DisputeResolutionDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->DisputeResolutionDocument->id = $id;
        if (!$this->DisputeResolutionDocument->exists()) {
            throw new NotFoundException(__('Invalid mpac document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->DisputeResolutionDocument->delete()) {
            $this->Session->setFlash(__('The mpac document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The mpac document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['DisputeResolutionDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 13));
