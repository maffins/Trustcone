...
class ExcoDocumentsController extends AppController {
  ...
  public $components = array('Paginator', 'Flash', 'Session');
    ...
    public function committeedetails() {
  	...
  	public function messageresend() {
  			if ($this->request->is('post')) {
  				$message = $this->request->data['ExcoDocument']['message'];
  				$subject = $this->request->data['ExcoDocument']['subject'];
  				...
  				if(!$message)
  				{
  					$message = "EXCO Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
  				}

  				if($this->request->data['ExcoDocument']['sendsms']){
  					 $this->sendsms($message);
  				}
  				if($this->request->data['ExcoDocument']['sendemail']){
  				...
  				return $this->redirect(array('Controller' => 'ExcoDocument', 'action' => 'committeedetails'));
  		}
  	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 2], 'order' => ['id' => 'DESC']]);
    ...
    public function view($id = null) {
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 2));
        ...
        $this->set('usertype', $this->Auth->user()['user_type_id']);
				 ...
				 */
				$this->loadModel('MeetingNotice');
				$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 2));
				 ...
				 */
				$this->loadModel('MeetingAddendum');
				$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 2));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['ExcoDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'exco_separatecovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 2;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['ExcoDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'exco_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 2;
            ...
            $notifications = $this->request->data['ExcoDocument']['notifications'];
            unset($this->request->data['ExcoDocument']['notifications']);

            $sendemail = $this->request->data['ExcoDocument']['sendemail'];
            unset($this->request->data['ExcoDocument']['sendemail']);

            $sendsms = $this->request->data['ExcoDocument']['sendsms'];
            unset($this->request->data['ExcoDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['ExcoDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['ExcoDocument']['message'], $this->request->data['ExcoDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
    ...
    function sendsms($message=null)
    {
        ...
        if(!$message)
        {
          $message = "EXCO Meeting documents are posted Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
        ...
        $mystring = $this->get_data($url);die;
    }
    ...
    function sendemail($message = null, $subject = null)
    {
        ...
        $users = $this->User->find('all');
				...
				if(!$subject)
				{
					 $subject = "EXCO Meeting documents are posted Meeting documents are posted";
				}
				if(!$message)
				{
					$message = "EXCO Meeting documents are posted Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
