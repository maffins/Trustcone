...
/**
 * AdhocCommittees Controller
 *
 * @property AdhocCommittee $AdhocCommittee
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class AdhocCommitteesController extends AppController {
	/**
	 * Components
	 *
	 * @var array
	...
	public function viewmembers() {
		...
		{
				if( in_array(79, unserialize($user['User']['permissions'])) )
				{
						$allmembers[] = $user;
				}
		}
	...
	public function index() {
			...
			$allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 22]]);
			...
			$auditTrail['AuditTrail']['event_description'] = "Opened the meetings page ";

			$auditTrail['AuditTrail']['contents'] = "Opened the meetings page ";
	...
	public function sendFile($id, $which) {
			...
			if($which == 5)
			{
					...
					$directory = "adhoc_separateCovers";
			}
			...
			if($which == 8)
			{
					...
					$directory = "adhoc_addendum";
			}

			$options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 22));
	...
	public function view($id = null) {
			...
			$options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 22));
			// $options = array('conditions' => array('MeetingAgenda.type' => 22));
			 ...
			 */
			$this->loadModel('MeetingAgenda');
			$options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 22));
			...
			$options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 22));
			...
			$options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 22));
			...
			$options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 22));
			...
			$options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 22));
			 ...
			 */
			$this->loadModel('MeetingNotice');
			$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 22));
			 ...
			 */
			$this->loadModel('MeetingAddendum');
			$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 22));
	...
	public function add() {
			...
			if ($this->request->is('post')) {
					...
					$meeting['Meeting']['name']    = $meetingName = $this->request->data['AdhocCommittee']['meeting'];
					$meeting['Meeting']['type']    = 22;
					...
					$previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 22),
					...
					$this->loadModel('MeetingAgenda');
					$agendaDocuments = $this->request->data['AdhocCommittee']['agenda'];
					$agendaDetails = [];
					...
					foreach( $agendaDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_agenda' . DS . $file['name']);
							...
							$agendaDetails['MeetingAgenda']['type']       = 22;
					...
					$this->loadModel('MeetingMinute');
					$minutesDocuments = $this->request->data['AdhocCommittee']['minutes'];
					$minutesDetails = [];
					...
					foreach( $minutesDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									$original_name = $file['name'];
									$file['name'] = preg_replace('/\s+/', '_', $file['name']);
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_minutes' . DS . $file['name']);
									//prepare the filename for database entry
							...
							$minutesDetails['MeetingMinute']['type']       = 22;
					...
					$this->loadModel('MeetingItem');
					$itemsDocuments = $this->request->data['AdhocCommittee']['items'];
					$itemsDetails = [];
					...
					foreach( $itemsDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									$original_name = $file['name'];
									$file['name'] = preg_replace('/\s+/', '_', $file['name']);
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_items' . DS . $file['name']);
									//prepare the filename for database entry
							...
							$itemsDetails['MeetingItem']['type']       = 22;
					...
					$this->loadModel('MeetingAttachment');
					$attachmentDocuments = $this->request->data['AdhocCommittee']['attachments'];
					$attachmentDetails = [];
					...
					foreach( $attachmentDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									$original_name = $file['name'];
									$file['name'] = preg_replace('/\s+/', '_', $file['name']);
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_attachments' . DS . $file['name']);
									//prepare the filename for database entry
							...
							$attachmentDetails['MeetingAttachment']['type']       = 22;
					...
					$this->loadModel('MeetingSeparatecover');
					$separateCoversDocuments = $this->request->data['AdhocCommittee']['separatecovers'];
					$separeCoverDetails = [];
					...
					foreach( $separateCoversDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									$original_name = $file['name'];
									$file['name'] = preg_replace('/\s+/', '_', $file['name']);
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_separateCovers' . DS . $file['name']);
									//prepare the filename for database entry
							...
							$separeCoverDetails['MeetingSeparatecover']['type']       = 22;
					...
					$this->loadModel('MeetingAddendum');
					$addendumDocuments = $this->request->data['AdhocCommittee']['addendum'];
					$addendumDetails = [];
					...
					foreach( $addendumDocuments as $upload)
					{
							...
							if ($meetingName) {
									...
									$original_name = $file['name'];
									$file['name'] = preg_replace('/\s+/', '_', $file['name']);
									move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'adhoc_addendum' . DS . $file['name']);
									//prepare the filename for database entry
							...
							$addendumDetails['MeetingAddendum']['type']       = 22;
				  ...
				  $auditTrail['AuditTrail']['userclient'] = $ExactBrowserNameBR;
					...
					$notifications = $this->request->data['AdhocCommittee']['notifications'];
					unset($this->request->data['AdhocCommittee']['notifications']);

					$sendemail = $this->request->data['AdhocCommittee']['sendemail'];
					unset($this->request->data['AdhocCommittee']['sendemail']);

					$sendsms = $this->request->data['AdhocCommittee']['sendsms'];
					unset($this->request->data['AdhocCommittee']['sendsms']);

          //This is for sending emails to others not part of counciler user type
					if($notifications) {
						if($sendsms) {
							$this->sendsms($this->request->data['AdhocCommittee']['message']);
						}
						if($sendemail) {
	            $this->sendemail($this->request->data['AdhocCommittee']['message'], $this->request->data['AdhocCommittee']['subject']);
						}
					} else {
						$this->sendsms();
						$this->sendemail();
					...
					$this->Flash->success(__('Uploaded successfuly.'));
					return $this->redirect(array('Controller' => 'AdhocCommittee', 'action' => 'index'));
			}
			...
			$users = $this->AdhocCommittee->User->find('list');
	...
	function sendsms($message=null)
	{
			...
			if(!$message)
			{
				$smsText = urlencode("ADHOC COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.");
      		} else {
	...
	public function messageresend() {
			if ($this->request->is('post')) {
				$message = $this->request->data['AdhocCommittee']['message'];
				$subject = $this->request->data['AdhocCommittee']['subject'];

				if(!$subject)
				{
					$subject = "Adhoc Committe Meeting documents are posted.";
				}
				if(!$message)
				{
					$message = "Adhoc Committe Meeting documents are posted. Please login to https://trustconetest.co.za/users/login to view documents.";
				}
				if($this->request->data['AdhocCommittee']['sendemail']){
						//$this->sendemailAttachment($message, $subject, $this->request->data);
						$this->sendemail($message, $subject );
				}
			
				if($this->request->data['AdhocCommittee']['sendsms']){
						$this->sendsms($message);
				}

				$this->Session->setFlash(__('Your messages have been sent.'), 'default', array('class' => 'alert alert-success'));

				return $this->redirect(array('Controller' => 'AdhocCommittee', 'action' => 'committeedetails'));
		}
	}
	...
	function sendemail($message=null, $subject=null)
	{
			...
			if(!$subject)
			{
				$subject = "ADHOC COMMITTEE Meeting documents are posted";
			}
			if(!$message)
			{
				$message = "Adhoc Meeting documents are posted. Please login to https://trustconetest.co.za/users/login to view documents.";
			}
	...
	public function cantdownload($id, $which) {
			...
			$this->loadModel('Meeting');
			$options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 22));
	...
	public function edit($id = null) {
			if (!$this->AdhocCommittee->exists($id)) {
					throw new NotFoundException(__('Invalid mpac document'));
			}
			if ($this->request->is(array('post', 'put'))) {
					if ($this->AdhocCommittee->save($this->request->data)) {
					...
					$options = array('conditions' => array('AdhocCommittee.' . $this->AdhocCommittee->primaryKey => $id));
					$this->request->data = $this->AdhocCommittee->find('first', $options);
			}
			$users = $this->AdhocCommittee->User->find('list');
			$this->set(compact('users'));
	}
	...
	public function delete($id = null) {
			$this->AdhocCommittee->id = $id;
			if (!$this->AdhocCommittee->exists()) {
					throw new NotFoundException(__('Invalid mpac document'));
			}
			$this->request->onlyAllow('post', 'delete');
			if ($this->AdhocCommittee->delete()) {
	...
	public function addmore($meet_id) {
			...
			if ($this->request->is('post')) {
					...
					$meeting_id = $this->request->data['AdhocCommittee']['meeting_id'];
			...
			$options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 22));
