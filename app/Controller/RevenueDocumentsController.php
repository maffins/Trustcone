...
/**
 * RevenueDocuments Controller
 *
 * @property RevenueDocument $RevenueDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class RevenueDocumentsController extends AppController {
    ...
    public function viewmembers() {
  		...
  		{
  				if( in_array(71, unserialize($user['User']['permissions'])) )
  				{
  						$allmembers[] = $user;
  				}
  		}
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['RevenueDocument']['message'];
    				$subject = $this->request->data['RevenueDocument']['subject'];

    				if(!$subject)
    				{
    					$subject = "REVENUE ENHANCEMENTS SECTION 79 COMMITTEE Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "REVENUE ENHANCEMENTS SECTION 79 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['RevenueDocument']['sendsms']){
    					 $this->sendsms($message);
    				}
    				if($this->request->data['RevenueDocument']['sendemail']){
    				...
    				return $this->redirect(array('Controller' => 'RevenueDocument', 'action' => 'committeedetails'));
    		}
    	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 21]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all Revenue Enhancements documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 8)
        {
            ...
            $directory = "revenue_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 21));
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view Revenue Enhancements document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing Revenue Enhancements document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 21));
        // $options = array('conditions' => array('MeetingAgenda.type' => 21));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 21));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 21));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 21));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 21));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 21));
        ...
        /*
    		 ...
    		 */
    		$this->loadModel('MeetingNotice');
    		$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 21));
    		 ...
    		 */
    		$this->loadModel('MeetingAddendum');
    		$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 21));
        ...
        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Viewing the meeting (Revenue Enhancements) details meeting id ".$id." ip ".$_SERVER['REMOTE_ADDR'];
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['RevenueDocument']['meeting'];
            $meeting['Meeting']['type']    = 21;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 21),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['RevenueDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 21;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['RevenueDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 21;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['RevenueDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 21;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['RevenueDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 21;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['RevenueDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 21;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['RevenueDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'revenue_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 21;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a Revenue Enhancements document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a Revenue Enhancements document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding Revenue Enhancements document');
            }

            $notifications = $this->request->data['RevenueDocument']['notifications'];
            unset($this->request->data['RevenueDocument']['notifications']);

            $sendemail = $this->request->data['RevenueDocument']['sendemail'];
            unset($this->request->data['RevenueDocument']['sendemail']);

            $sendsms = $this->request->data['RevenueDocument']['sendsms'];
            unset($this->request->data['RevenueDocument']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['RevenueDocument']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['RevenueDocument']['message'], $this->request->data['RevenueDocument']['subject']);
              }
            }else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'RevenueDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add Revenue Enhancements add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add Revenue Enhancements document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing Revenue Enhancements document');
        }

        $users = $this->RevenueDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 21));
    ...
    public function edit($id = null) {
        if (!$this->RevenueDocument->exists($id)) {
            throw new NotFoundException(__('Invalid Revenue Enhancements document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->RevenueDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The Revenue Enhancements document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The Revenue Enhancements document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('RevenueDocument.' . $this->RevenueDocument->primaryKey => $id));
            $this->request->data = $this->RevenueDocument->find('first', $options);
        }
        $users = $this->RevenueDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->RevenueDocument->id = $id;
        if (!$this->RevenueDocument->exists()) {
            throw new NotFoundException(__('Invalid Revenue Enhancements document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->RevenueDocument->delete()) {
            $this->Session->setFlash(__('The Revenue Enhancements document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The Revenue Enhancements document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['RevenueDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 21));
