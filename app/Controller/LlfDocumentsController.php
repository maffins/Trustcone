...
/**
 * LlfDocumentss Controller
 *
 * @property LlfDocuments $LlfDocuments
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class LlfDocumentsController extends AppController {
    ...
    public function viewmembers() {
      ...
      {
          if( in_array(20, unserialize($user['User']['permissions'])) )
          {
              $allmembers[] = $user;
          }
      }
      ...
      public function committeedetails() {
    	...
    	public function messageresend() {
    			if ($this->request->is('post')) {
    				$message = $this->request->data['LlfDocuments']['message'];
    				$subject = $this->request->data['LlfDocuments']['subject'];

    				if(!$subject)
    				{
    					$subject = "LLF Committee Section 80 Meeting documents are posted.";
    				}
    				if(!$message)
    				{
    					$message = "LLF Committee Section 80 Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
    				}

    				if($this->request->data['LlfDocuments']['sendemail']){
    					 $this->sendemail($message, $subject);
    				}
            if($this->request->data['LlfDocuments']['sendsms']){
    					 $this->sendsms($message);
    				}
    				$this->Session->setFlash(__('Your messages have been sent.'), 'default', array('class' => 'alert alert-success'));

    				return $this->redirect(array('Controller' => 'LlfDocuments', 'action' => 'committeedetails'));
    		}
    	}
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 25]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all LLF documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 8)
        {
            ...
            $directory = "llf_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 25));
        //print_r($options);
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view LLF document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing LLF document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 25));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 25));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 25));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 25));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 25));
   ...
   */
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 25));
        ...
        /*
    		 ...
    		 */
    		$this->loadModel('MeetingNotice');
    		$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 25));
    		 ...
    		 */
    		$this->loadModel('MeetingAddendum');
    		$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 25));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['LlfDocuments']['meeting'];
            $meeting['Meeting']['type']    = 25;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 25),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['LlfDocuments']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 25;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['LlfDocuments']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 25;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['LlfDocuments']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 25;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['LlfDocuments']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 25;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['LlfDocuments']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_separatecovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 25;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['LlfDocuments']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'llf_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 25;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a LLF document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a LLF document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding LLF document');
            }

            $notifications = $this->request->data['LlfDocuments']['notifications'];
            unset($this->request->data['LlfDocuments']['notifications']);

            $sendemail = $this->request->data['LlfDocuments']['sendemail'];
            unset($this->request->data['LlfDocuments']['sendemail']);

            $sendsms = $this->request->data['LlfDocuments']['sendsms'];
            unset($this->request->data['LlfDocuments']['sendsms']);

            //This is for sending emails to others not part of counciler user type
            if($notifications) {
              if($sendsms) {
                $this->sendsms($this->request->data['LlfDocuments']['message']);
              }
              if($sendemail) {
                $this->sendemail($this->request->data['LlfDocuments']['message'], $this->request->data['LlfDocuments']['subject']);
              }
            } else {
              $this->sendsms();
              $this->sendemail();
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'LlfDocuments', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add LLF add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add LLF document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing LLF document');
        }
    ...
    function sendsms($message = null)
    {
        ...
        $numbers .= '27817549884,27716435674';

        if(!$message)
        {
          $message = "LLF documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    function sendemail($message = null, $subject = null)
    {
        ...
        if(!$subject)
        {
          $subject = "LLF Committee documents are posted";
        }
        if(!$message)
        {
          $message = "LLF Committee documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
        }
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 25));
        ...
        $Email->from(array('no-reply@documentstracker.com' => 'Matjhabeng Local Municipality Document Management System'))
            ->template('cantdownload_LLF', 'default')
            ->emailFormat('html')
    ...
    public function edit($id = null) {
        if (!$this->LlfDocument->exists($id)) {
            throw new NotFoundException(__('Invalid LLF document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->LlfDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The LLF document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The LLF document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('LlfDocuments.' . $this->LlfDocument->primaryKey => $id));
            $this->request->data = $this->LlfDocument->find('first', $options);
        }
        $users = $this->LlfDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->LlfDocument->id = $id;
        if (!$this->LlfDocument->exists()) {
            throw new NotFoundException(__('Invalid LLF document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->LlfDocument->delete()) {
            $this->Session->setFlash(__('The LLF document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The LLF document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['LlfDocuments']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 25));
