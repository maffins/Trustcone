...
/**
 * ChairDocuments Controller
 *
 * @property ChairDocument $ChairDocument
 * @property PaginatorComponent $Paginator
 * @property FlashComponent $Flash
 * @property SessionComponent $Session
 */
class ChairDocumentsController extends AppController {
    /**
     * Components
     *
     * @var array
    ...
    public function committeedetails() {
  	...
  	public function messageresend() {
  			if ($this->request->is('post')) {
  				$message = $this->request->data['ChairDocument']['message'];
  				$subject = $this->request->data['ChairDocument']['subject'];

  				if(!$subject)
  				{
  					$subject = "CHAIR OF CHAIRS SECTION 79 COMMITTEE Meeting documents are posted.";
  				}
  				if(!$message)
  				{
  					$message = "CHAIR OF CHAIRS SECTION 79 COMMITTEE Meeting documents are posted. Please login to http://trustconetest.co.za/users/login to view documents.";
  				}

  				if($this->request->data['ChairDocument']['sendsms']){
  					 $this->sendsms($message);
  				}
  				if($this->request->data['ChairDocument']['sendemail']){
  				...
  				return $this->redirect(array('Controller' => 'ChairDocument', 'action' => 'committeedetails'));
  		}
  	}
    ...
    public function viewmembers() {
      ...
      {
          if( in_array(62, unserialize($user['User']['permissions'])) )
          {
              $allmembers[] = $user;
          }
      }
    ...
    public function index() {
        ...
        $allmeetings = $this->Meeting->find('all', ['fields'=>'DISTINCT name, idcounter, created, user_id', 'conditions'=>['Meeting.type' => 17]]);
        ...
        {
            die('There was a problem trying to save the audit trail for viewing all CHAIR OF CHAIRS documents');
        }
    }
    ...
    public function sendFile($id, $which) {
        ...
        if($which == 5)
        {
            ...
            $directory = "chair_separateCovers";
        }
        ...
        if($which == 8)
        {
            ...
            $directory = "chair_addendum";
        }

        $options = array('conditions' => array($tafura.'.' . $this->$tafura->primaryKey => $id, $tafura.'.type' => 17));
        ...
        $auditTrail['AuditTrail']['contents'] = "Opened to view CHAIR OF CHAIRS document with id ".$id." item ".$directory." ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for viewing CHAIR OF CHAIRS document');
        }
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $id, 'Meeting.type' => 17));
        // $options = array('conditions' => array('MeetingAgenda.type' => 17));
         ...
         */
        $this->loadModel('MeetingAgenda');
        $options1 = array('conditions' => array('MeetingAgenda.meeting_id' => $id, 'MeetingAgenda.type' => 17));
        ...
        $options2 = array('conditions' => array('MeetingMinutes.meeting_id' => $id, 'MeetingMinutes.type' => 17));
        ...
        $options3 = array('conditions' => array('MeetingItems.meeting_id' => $id, 'MeetingItems.type' => 17));
        ...
        $options4 = array('conditions' => array('MeetingAttachments.meeting_id' => $id, 'MeetingAttachments.type' => 17));
        ...
        $options5 = array('conditions' => array('MeetingSeparatecovers.meeting_id' => $id, 'MeetingSeparatecovers.type' => 17));
        ...
        $this->set('usertype', $this->Auth->user()['user_type_id']);
				 ...
				 */
				$this->loadModel('MeetingNotice');
				$options6 = array('conditions' => array('MeetingNotice.meeting_id' => $id, 'MeetingNotice.type' => 17));
				 ...
				 */
				$this->loadModel('MeetingAddendum');
				$options7 = array('conditions' => array('MeetingAddendum.meeting_id' => $id, 'MeetingAddendum.type' => 17));
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting['Meeting']['name']    = $meetingName = $this->request->data['ChairDocument']['meeting'];
            $meeting['Meeting']['type']    = 17;
            ...
            $previousMeeting = $this->Meeting->find('first', array('conditions' => array('Meeting.type' => 17),
            ...
            $this->loadModel('MeetingAgenda');
            $agendaDocuments = $this->request->data['ChairDocument']['agenda'];
            $agendaDetails = [];
            ...
            foreach( $agendaDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_agenda' . DS . $file['name']);
                ...
                $agendaDetails['MeetingAgenda']['type']       = 17;
            ...
            $this->loadModel('MeetingMinute');
            $minutesDocuments = $this->request->data['ChairDocument']['minutes'];
            $minutesDetails = [];
            ...
            foreach( $minutesDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_minutes' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $minutesDetails['MeetingMinute']['type']       = 17;
            ...
            $this->loadModel('MeetingItem');
            $itemsDocuments = $this->request->data['ChairDocument']['items'];
            $itemsDetails = [];
            ...
            foreach( $itemsDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_items' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $itemsDetails['MeetingItem']['type']       = 17;
            ...
            $this->loadModel('MeetingAttachment');
            $attachmentDocuments = $this->request->data['ChairDocument']['attachments'];
            $attachmentDetails = [];
            ...
            foreach( $attachmentDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_attachments' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $attachmentDetails['MeetingAttachment']['type']       = 17;
            ...
            $this->loadModel('MeetingSeparatecover');
            $separateCoversDocuments = $this->request->data['ChairDocument']['separatecovers'];
            $separeCoverDetails = [];
            ...
            foreach( $separateCoversDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_separateCovers' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $separeCoverDetails['MeetingSeparatecover']['type']       = 17;
            ...
            $this->loadModel('MeetingAddendum');
            $addendumDocuments = $this->request->data['ChairDocument']['addendum'];
            $addendumDetails = [];
            ...
            foreach( $addendumDocuments as $upload)
            {
                ...
                if ($meetingName) {
                    ...
                    $original_name = $file['name'];
                    $file['name'] = preg_replace('/\s+/', '_', $file['name']);
                    move_uploaded_file($file['tmp_name'], WWW_ROOT . 'uploads' . DS . 'chair_addendum' . DS . $file['name']);
                    //prepare the filename for database entry
                ...
                $addendumDetails['MeetingAddendum']['type']       = 17;
            ...
            $this->loadModel('AuditTrail');
            $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
            $auditTrail['AuditTrail']['event_description'] = "Adding a CHAIR OF CHAIRS document ".$_SERVER['REMOTE_ADDR'];

            $auditTrail['AuditTrail']['contents'] = "Adding a CHAIR OF CHAIRS document ".$_SERVER['REMOTE_ADDR'];
            if( !$this->AuditTrail->save($auditTrail))
            {
                die('There was a problem trying to save the audit trail for adding CHAIR OF CHAIRS document');
            }

            $notifications = $this->request->data['ChairDocument']['notifications'];
  					unset($this->request->data['ChairDocument']['notifications']);

  					$sendemail = $this->request->data['ChairDocument']['sendemail'];
  					unset($this->request->data['ChairDocument']['sendemail']);

  					$sendsms = $this->request->data['ChairDocument']['sendsms'];
  					unset($this->request->data['ChairDocument']['sendsms']);
            ...
            $this->Flash->success(__('Uploaded successfuly.'));
            return $this->redirect(array('Controller' => 'ChairDocument', 'action' => 'index'));
        }

        $this->loadModel('AuditTrail');
        $auditTrail['AuditTrail']['user_id'] = $this->Auth->user('id');
        $auditTrail['AuditTrail']['event_description'] = "Open the add CHAIR OF CHAIRS add page ".$_SERVER['REMOTE_ADDR'];

        $auditTrail['AuditTrail']['contents'] = "Open the add CHAIR OF CHAIRS document page ".$_SERVER['REMOTE_ADDR'];
        if( !$this->AuditTrail->save($auditTrail))
        {
            die('There was a problem trying to save the audit trail for editing CHAIR OF CHAIRS document');
        }

        $users = $this->ChairDocument->User->find('list');
    ...
    public function cantdownload($id, $which) {
        ...
        $this->loadModel('Meeting');
        $options1 = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $file[$tafura]['meeting_id'], 'Meeting.type' => 17));
    ...
    public function edit($id = null) {
        if (!$this->ChairDocument->exists($id)) {
            throw new NotFoundException(__('Invalid CHAIR OF CHAIRS document'));
        }
        if ($this->request->is(array('post', 'put'))) {
            if ($this->ChairDocument->save($this->request->data)) {
                $this->Session->setFlash(__('The CHAIR OF CHAIRS document has been saved.'), 'default', array('class' => 'alert alert-success'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('The CHAIR OF CHAIRS document could not be saved. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
            }
        } else {
            $options = array('conditions' => array('ChairDocument.' . $this->ChairDocument->primaryKey => $id));
            $this->request->data = $this->ChairDocument->find('first', $options);
        }
        $users = $this->ChairDocument->User->find('list');
        $this->set(compact('users'));
    }
    ...
    public function delete($id = null) {
        $this->ChairDocument->id = $id;
        if (!$this->ChairDocument->exists()) {
            throw new NotFoundException(__('Invalid CHAIR OF CHAIRS document'));
        }
        $this->request->onlyAllow('post', 'delete');
        if ($this->ChairDocument->delete()) {
            $this->Session->setFlash(__('The CHAIR OF CHAIRS document has been deleted.'), 'default', array('class' => 'alert alert-success'));
        } else {
            $this->Session->setFlash(__('The CHAIR OF CHAIRS document could not be deleted. Please, try again.'), 'default', array('class' => 'alert alert-danger'));
        }
        return $this->redirect(array('action' => 'index'));
    }
    ...
    public function addmore($meet_id) {
        ...
        if ($this->request->is('post')) {
            ...
            $meeting_id = $this->request->data['ChairDocument']['meeting_id'];
        ...
        $options = array('conditions' => array('Meeting.' . $this->Meeting->primaryKey => $meet_id, 'Meeting.type' => 17));
