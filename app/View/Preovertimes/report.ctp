...
<div class="preovertimes index">
	...
	<div class="row">
		<div class="col-md-12">
			<div class="page-header">
				<h1><?php echo __('Pre Overtime Managers Report') ?></h1>
        <h4>
          <?php if($who == 4):?><b>All directorates and units</b><?php endif;?>
          <?php if($who == 2):?>Directorate: <b><?php echo $maindetails['Department']['name']; ?></b><?php endif;?>
          <?php if($who == 1):?>
	...
	<div class="row">
		...
		<div class="col-md-12">
            ...
            <?php echo $this->Html->link('Generate Summary', array('controller' => 'Preovertimes', 'action' => 'mgrsummary'), array('escape' => false)); ?>
						</div><!-- end body -->
				</div><!-- end panel -->
			</div><!-- end actions -->
		</div><!-- end col md 3 -->
					...
					<?php foreach ($preovertimes as $preovertime): ?>
						<?php if($preovertime['Preovertime']['total_hours']):?>
							<tr>
								...
								<td ><?php
                  if($who == 1 || $who == 2){
                    if( $preovertime['Preovertime']['tracker'] != 10 ) { echo 'Approved'; } else { echo 'Declined'; }
                  }
                   ?></td>
